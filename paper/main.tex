...
\item \textbf{自适应音乐引擎（Adaptive Music Engine）}：实现为可插拔的\texttt{AudioBackend}接口，包含两种后端：（A）\textbf{占位混音器}（\texttt{PlaceholderMixer}，默认）——基于NumPy与\texttt{sounddevice}，在Python进程内从各情境\textbf{专辑}中随机抽取一首歌曲循环播放（MP3/WAV），并按亮度、能量、温暖度做增益整形与\textbf{等功率（equal-power）交叉淡化}；解码与缓冲安装在音频回调锁之外完成，启动后后台预热其余专辑，避免切换时 underrun 静音间隙；（B）\textbf{Godot 4侧载}（\texttt{GodotAudioBackend}）——Python主进程通过本地TCP JSON API控制Godot中的\texttt{AudioStreamPlayer}。默认资产结构为每个情境目录下的歌曲专辑；若目录内仍为四层同步stem（\texttt{ambient}、\texttt{rhythm}、\texttt{harmonic}、\texttt{accent}）则启用分层混音。四层stem的声学角色为：
\begin{itemize}
...
\texttt{assets/audio/} & 各情境专辑目录（\texttt{\{profile\}/*.mp3|wav}），运行时随机抽曲；可选 Godot 四层 stem \\
...
\item \textbf{音频执行}：若用户已点击 Start Audio 且需要过渡，调用 \texttt{audio.crossfade\_to()}；占位混音器在锁外解码后于 \texttt{sounddevice} 回调中做等功率双 profile 混合，Godot 后端则经 TCP 发送 \texttt{crossfade} 指令。
...
\begin{tikzpicture}[
    ...
    \node[flowbox, below=of decide] (audio) {6.\ 若用户已 Start Audio 且 \texttt{should\_transition}：\\ \texttt{audio.crossfade\_to()} $\rightarrow$ \texttt{PlaceholderMixer}（sounddevice）或 Godot TCP \texttt{crossfade}};
    \node[flowbox, below=of audio] (ui) {7.\ \texttt{MainWindow} 刷新情境/专注/Profile 卡片；重置窗口切换计数\\ $\rightarrow$ 等待下一周期（循环至步骤 1）};
...
\textbf{（2）情境专辑与无间隙过渡。}各情境对应\texttt{assets/audio/\{profile\_id\}/}下的\textbf{歌曲专辑}（MP3/WAV）；切换至该情境时从专辑中\textbf{随机抽取}一首循环播放。首次启动可将旧版扁平\texttt{\{profile\}.mp3}迁移为专辑内单曲。用户可在界面“Manage Albums”中上传或删除曲目。占位混音器将所选歌曲解码为单声道循环缓冲；新曲解码在\texttt{sounddevice}回调锁之外进行，播放头跳过循环边界淡入区，并以等功率曲线混合双缓冲，避免过渡瞬间的静音 underrun。Godot侧载优先播放专辑随机曲；若目录内仍为四层stem则回退至分层混音。
...
界面采用深色极简PyQt6实现，非干扰式展示：当前检测情境、专注状态与分数条、活动声景配置及交叉淡化进度。用户可\textbf{手动锁定情境/声景}、调节灵敏度、打开\textbf{类别配置}对话框维护各情境的进程名/标题关键词、在\textbf{Manage Albums}中为各情境上传/删除专辑曲目，并配置隐私选项。遇到未识别窗口时，右下角确认条允许一键将推断结果写入用户词典。系统托盘支持后台静默运行，使用户无需主动“选歌”或编写prompt即可获得与任务匹配的声景。
