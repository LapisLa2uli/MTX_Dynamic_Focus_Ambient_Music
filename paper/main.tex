...
\item \textbf{情境分类器（Context Classifier）}：采用\textbf{基于规则的启发式分类}，将\textbf{前台进程名}与\textbf{窗口标题}（均为可选采集的元数据，不含按键内容）映射至六类工作情境——编程/工程、团队/截止期协作、阅读/写作、科学/数学工作、创意设计、分心状态——并输出置信度$\gamma$。分类器\textbf{不}使用键鼠计数、CPU 或空闲时长；后者仅用于认知估计。对未命中内置词典的“杂项/unknown”窗口，轻量\texttt{ContextInferer}基于种子词表与词元重叠给出候选情境；系统在屏幕右下角弹出确认条，用户可确认或纠正并写入本地\texttt{user\_context\_mappings.json}。用户亦可在界面中按类别配置进程名与标题关键词。新情境须持续\textbf{驻留$T_d$}（默认 8--30\,s，随$\gamma$自适应，可配置）方被采纳，并对短暂 \texttt{unknown} 标签设置宽限，以抑制抖动。
...
\paragraph{驻留滤波与手动覆盖。}\texttt{ContextPersistence} 在候选情境连续稳定后才更新 \texttt{WorkContext}：当 $\gamma\ge 0.75$ 时 $T_d=T_{d,\min}=8$\,s；当 $\gamma\ge 0.5$ 时 $T_d=15$\,s（默认）；否则 $T_{d,\max}=30$\,s。置信度 $\gamma<0.25$ 的样本视为 \texttt{unknown}；若当前已处于某生产性情境，连续 4 个周期的 \texttt{unknown} 才允许清空标签，以避免 Alt-Tab 造成的瞬时误判。用户启用 UI \textbf{手动覆盖}时，跳过上述规则，直接采用所选情境且 $\gamma=1$。
...
\item \textbf{情境推断}：\texttt{classify\_snapshot()} 仅依据前台\textbf{进程名}与\textbf{窗口标题}对六类工作情境及 \texttt{distraction} 打分（见表~\ref{tab:activity-snapshot}--\ref{tab:context-detection}）；\texttt{ContextPersistence} 要求新标签持续 $T_d\in[8,30]$\,s（随 $\gamma$ 自适应）才切换，并对短暂 \texttt{unknown} 设宽限。用户启用\textbf{手动覆盖}时，直接采用 UI 所选情境且置信度为 1。
...
\begin{tikzpicture}[
    ...
    \node[flowbox, below=of poll] (ctx) {3.\ \texttt{classify\_snapshot()}：进程名+标题规则打分；\texttt{ContextPersistence} 驻留 $T_d\in[8,30]$\,s\\ $\rightarrow$ \texttt{WorkContext}（可选手动覆盖）};
...
$T_{d,\min}$ & 高置信度情境最短驻留（默认 8\,s） & $\mathbb{R}^+$ \\
$T_{d,\max}$ & 低置信度情境最长驻留（默认 30\,s） & $\mathbb{R}^+$ \\
